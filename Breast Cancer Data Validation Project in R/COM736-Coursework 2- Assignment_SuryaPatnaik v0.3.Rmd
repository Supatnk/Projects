...
```{r}
setwd('C:\\Users\\Ranjita\\Documents\\00_To Do\\COM736 ( 45947 ) - Data Valid and Visual. 2017-18\\Assessment\\COM736-Coursework 2- Assignment')
...
```
  Use hmisc to perform imputation 
  
```{r}
library(Hmisc)

cancer_nona <- Cancer_OutlierNA
Cancer_imputed_Marginal.Adhesion <- impute(cancer_nona$Marginal.Adhesion, mean)
Cancer_imputed_Epithelial.Cell.Size <- impute(cancer_nona$Epithelial.Cell.Size, mean)
Cancer_imputed_Normal.Nucleoli <- impute(cancer_nona$Normal.Nucleoli, mean)
Cancer_imputed_Bland.Chromatin_outlier <- impute(cancer_nona$Bland.Chromatin, mean)
Cancer_imputed_Mitoses_outlier <- impute(cancer_nona$Mitoses, mean)
...
#is.imputed(Cancer_imputed_Marginal.Adhesion)
#is.imputed(Cancer_imputed_Marginal.Adhesion)


library(ggplot2)
df <-data.frame(cancer_nona)
ggplot(stack(df[,2:10]), aes(x = ind, y = values)) + geom_boxplot() + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1, vjust=1)) +
  labs(title = "Boxplots of columns") + labs(x = "", y = "Values") + 
  scale_y_continuous(breaks = seq(1, 10, by = 1))
