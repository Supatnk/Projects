...
# Use the fit() function to train the model for 100 epochs using batches of 5 :
history <- model %>% fit(
  X_train, y_train, 
...
###################### (6) Generalized Linear Model  Regression Start #####################
# library(ggplot)
# 
# model_glm <- caret::train(diagnosis ~ .,
#                           data = DeveSubset,
#                           method = "glm",
...
# model_glm
# 
# predictions <- predict(model_glm, TestingSubset)
# 
# data.frame(actual = TestingSubset$diagnosis,predicted = predictions) %>%
